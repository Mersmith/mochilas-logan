...
new #[Title('Mantenimiento de Atributos')] class extends Component {
    use WithPagination;
    ...
    public function eliminar(int $id): void
    {
        ...
        $atributo = Atributo::withTrashed()->findOrFail($id);

        if ($atributo->trashed()) {
            $atributo->forceDelete();
    ...
    {
        $query = Atributo::query()->orderBy('nombre', 'asc');
        return Excel::download(new AtributosExport($query), 'todos_los_atributos.xlsx');
    }

    public function exportarFiltrados()
    {
        $query = $this->getBaseQuery();
        return Excel::download(new AtributosExport($query), 'atributos_filtrados.xlsx');
    }
}; ?>
...
<div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <flux:subheading>{{ __('Administra los atributos para las variaciones de productos (colores, tallas).') }}</flux:subheading>
        </div>
        <div class="flex flex-wrap items-center gap-2">
            ...
            @can('atributos.editar')
                <flux:button variant="primary" icon="plus" href="{{ route('admin.atributos.create') }}" wire:navigate>
                    {{ __('Nuevo Atributo') }}
                </flux:button>
            @endcan
        </div>
    ...
    <div class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl shadow-sm overflow-hidden flex flex-col">
        <div class="overflow-x-auto flex-1">
            <table class="w-full text-left border-collapse text-sm">
                ...
                <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                    @forelse($this->atributos as $atributo)
                        <tr class="hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30 transition-colors {{ $atributo->trashed() ? 'opacity-50' : '' }}">
                                    ...
                                    <div class="flex items-center justify-end gap-2">
                                        @if($atributo->trashed())
                                            <flux:button variant="ghost" icon="arrow-path" size="sm"
                                                wire:click.prevent="restaurar({{ $atributo->id }})"
                                            ...
                                            <flux:button variant="ghost" icon="trash" size="sm" class="text-red-500 hover:text-red-600"
                                                wire:click.prevent="confirmarEliminacion({{ $atributo->id }}, true)" />
                                        @else
                                            <flux:button variant="ghost" icon="pencil-square" size="sm"
                                                href="{{ route('admin.atributos.edit', $atributo->id) }}" wire:navigate />
                                            <flux:button variant="ghost" icon="trash" size="sm" class="text-red-500 hover:text-red-600"
                                                wire:click.prevent="confirmarEliminacion({{ $atributo->id }})" />
                                        @endif
                                    </div>
                                </td>
                            @endcan
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ auth()->user()->can('atributos.editar') ? 3 : 2 }}"
        ...
        @else
            <div class="px-4 py-3 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between text-xs text-zinc-400">
                <span>{{ $this->atributos->total() }} {{ __('registro(s) encontrado(s)') }}</span>
            </div>
        @endif
    </div>
