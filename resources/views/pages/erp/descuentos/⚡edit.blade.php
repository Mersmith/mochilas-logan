...
new #[Title('Editar Descuento')] class extends Component {
    ...
    public function mount(Descuento $descuento)
    {
        $this->descuento = $descuento;
        $this->nombre = $descuento->nombre;
        $this->porcentaje_descuento = (string) $descuento->porcentaje_descuento;
        $this->fecha_inicio = $descuento->fecha_inicio ? $descuento->fecha_inicio->format('Y-m-d\TH:i') : '';
        $this->fecha_fin = $descuento->fecha_fin ? $descuento->fecha_fin->format('Y-m-d\TH:i') : '';
        $this->activo = $descuento->activo;
        
        // Cargar productos seleccionados previamente
        $this->productos_seleccionados = $descuento->productos()->pluck('productos.id')->map(fn($id) => (string) $id)->toArray();
    }
    ...
    {
        if (! auth()->user()->can('promociones.editar')) {
            abort(403);
        }
        ...
        $this->descuento->productos()->sync($this->productos_seleccionados);

        Flux::toast(variant: 'success', text: 'Descuento actualizado correctamente.');
...
<div class="space-y-6 max-w-5xl mx-auto">
    <div class="flex items-center justify-between">
        <div>
            <flux:heading size="xl">{{ __('Editar Descuento') }}</flux:heading>
            <flux:subheading>{{ __('Modifica los datos del descuento y asigna productos.') }}</flux:subheading>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-1">
            <form class="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-700 rounded-xl p-6 shadow-sm space-y-5 h-fit">
                ...
                <flux:field>
                    <flux:label>{{ __('Porcentaje (%)') }}</flux:label>
                ...
                <flux:switch wire:model="activo" label="Descuento Activo" description="Inactivos no se aplican." />
